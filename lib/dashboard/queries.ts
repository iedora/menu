...
// references inside subqueries — interpolating ${table.column} in a correlated
// subquery rendered as a bare `"column"` and silently resolved against the
// inner FROM, producing 0 for every row. (See commit 798d09e.) These queries
// are written as separate small queries merged in JS rather than a single
// CROSS JOIN with COUNT(DISTINCT ...) so the SQL stays simple and reviewable.
//
 ...
 */
export async function getOrganizationRestaurantsWithCounts(
...
/**
 * Lists the menus inside a restaurant with category and dish counts. Caller
 * must have already verified the caller's access to `restaurantId` (e.g.
 * via `requireRestaurantAccess` / `requireRestaurantBySlug`).
 */
export async function getRestaurantMenusWithCounts(
  restaurantId: string,
