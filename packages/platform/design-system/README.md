...
Editorial design system for Iedora products. Paper, ink, cinnabar; Playfair Display, Lora, Geist + Geist Mono; hairline rules; one warm accent used sparingly. Every product in the monorepo (house, menu) renders out of this one package.
...
The visual chrome is identical across products only if both products load the same Playfair Display + Lora + Geist + Geist Mono cuts. Two surfaces, one contract:

| Font | Required cuts | Role |
|---|---|---|
| **Playfair Display** | weights 400/500/600/700 in both `normal` and `italic` | `--display` — wordmark + h1 |
| **Lora** | weights 400/500/600 in both `normal` and `italic` | `--serif` — eyebrows, course voice, body italic |
| **Geist** | weights 400/500/600 | `--sans` — UI controls + body |
| **Geist Mono** | weights 400/500 | `--mono` — labels, step counter, meta |

If italic isn't loaded, `<em>` and the editorial italic body text fall back to a browser faux-slant — visibly off from the real Playfair/Lora italic cuts that the Wordmark + the headlines rely on.

**Astro / static HTML** — load via `<link>`:

```html
<link
  href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500;1,600&family=Lora:ital,wght@0,400;0,500;1,400;1,500;1,600&family=Geist:wght@400;500;600&family=Geist+Mono:wght@400;500&display=swap"
  rel="stylesheet"
/>
```
...
<html
  className={`${geistSans.variable} ${geistMono.variable} ${playfair.variable} ${lora.variable}`}
  style={{
    ["--display" as string]: "var(--font-playfair), Georgia, serif",
    ["--serif" as string]:   "var(--font-lora), Georgia, serif",
    ["--sans" as string]:    "var(--font-geist-sans), 'Helvetica Neue', Arial, sans-serif",
    ["--mono" as string]:    "var(--font-geist-mono), ui-monospace, monospace",
  }}
>
```
...
| Layer | Tokens |
|---|---|
| **Palette** | `--paper` `--paper-2` `--paper-3` `--card` `--card-2` · `--ink` `--ink-2` + opacity ladder `--ink-04 / 08 / 14 / 22 / 40 / 55 / 70 / 85` · `--muted` `--muted-2` · `--rule` `--rule-2` · `--cinnabar` `--cinnabar-deep` `--cinnabar-08` `--cinnabar-16` |
| **Typography** | `--display` (Playfair Display) · `--serif` (Lora italic) · `--sans` (Geist) · `--mono` (Geist Mono) |
