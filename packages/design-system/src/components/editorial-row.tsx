      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-3">
            {row.index && (
              <span aria-hidden="true" className="font-serif text-[12.5px] italic text-muted-foreground tabular-nums">
                {row.index}
              </span>
            )}
            ...
            {trailing ? (
              <div className="text-right shrink-0">
                <div className="text-[15px] font-medium tabular-nums">
                  {trailing.value === null && !trailing.label ? (
                    <span className="text-muted-foreground">—</span>
                  ) : (
                    <>
                      {trailing.label ?? trailing.value?.toLocaleString()}
                      {trailing.deltaPct !== undefined && <DeltaTag deltaPct={trailing.deltaPct} />}
                    </>
                  )}
                </div>
                {trailing.comparison && (
                  <div className="font-serif text-[12px] italic text-muted-foreground mt-1">{trailing.comparison}</div>
                )}
              </div>
            ) : null}
          </div>
          {(row.metadata || (row.actions && row.actions.length > 0) || row.extraActions) && (
            <div className={styles.subRow}>
              {row.metadata && <span className="font-serif italic text-muted-foreground">{row.metadata}</span>}
              {((row.actions && row.actions.length > 0) || row.extraActions) && (
                <span className="inline-flex flex-wrap items-center gap-2 ml-auto">
                  {row.actions?.map((a) => <ActionChip key={a.key} action={a} />)}
                  {row.extraActions}
                </span>
              )}
            </div>
...
function DeltaTag({ deltaPct }: { deltaPct: number }) {
  ...
  return (
    <span className={'ml-2 text-[12px] tabular-nums ' + (positive ? 'text-[#3d5a3a]' : negative ? 'text-[#9c4f3f]' : 'text-muted-foreground')}>
      {marker}{value}
    </span>
  )
}
