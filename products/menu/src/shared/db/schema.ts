...
//   - `auth.*`  — owned by Genkan; menu redeclares the same shapes so its
//                 Drizzle/Better Auth instance can read/write the shared
//                 identity tables (one DATABASE_URL, no two-DB plumbing).
//   - `menu.*`  — owned by this product; every domain table lives here.
//
// Migrations in BOTH apps emit `CREATE SCHEMA IF NOT EXISTS auth` so order
// of application doesn't matter — whichever runs first creates the schema,
// the other no-ops. Only menu's migrations emit `CREATE SCHEMA … menu`.
...
// Schema generated by `bun run auth:generate` against /tmp/ba-schema.ts.
