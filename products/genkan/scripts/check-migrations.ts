 ...
 *
 * Note: drizzle-kit writes a single shared `drizzle.__drizzle_migrations`
 * tracking table when applying migrations, with one row per applied
 * migration. Both genkan and menu append rows there with their own tag
 * timestamps; the journal is per-app so each `check` only looks at its own
 * entries.
 */
...
async function main() {
  ...
  try {
    // Per-product tracker — see drizzle.config.ts. Genkan's lives in
    // `auth.__drizzle_migrations` so it doesn't shadow menu's tracker.
    const tableRows = await sql<{ exists: boolean }[]>`
      SELECT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'auth' AND table_name = '__drizzle_migrations'
      ) AS exists
    `
    ...
    const applied = hasTable
      ? await sql<{ created_at: string }[]>`
          SELECT created_at FROM auth.__drizzle_migrations
        `
      : []
